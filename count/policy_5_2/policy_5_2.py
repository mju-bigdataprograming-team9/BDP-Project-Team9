# 기존에는 단어만 카운트하고 구(phrase)는 카운트하고 있지 않아서 해당 부분 수정 
        ...
        for title in titles:
            for policy_id, policy_words in policy_labels.items():
                for word in policy_words:
                    ...
                    policy_totals[policy_id] += count * title_weight
        
        for article in articles:
            for policy_id, policy_words in policy_labels.items():
                for word in policy_words:
                    count = len(re.findall(re.escape(word), article))
                    policy_totals[policy_id] += count * body_weight
...
# 결과를 txt 파일로 저장
output_file_path = "policy_5_2_results_with_counts.txt"
