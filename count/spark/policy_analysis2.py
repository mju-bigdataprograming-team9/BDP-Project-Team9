...
from pyspark.sql.functions import col, lit, sum as spark_sum, length, expr
...
input_path = "hdfs:///user/maria_dev/crawler/merged_data/merged_201911.csv"
...
# 정책별 키워드 등장 횟수 계산 (replace 사용)
for policy_id, words in policy_labels.items():
    count_exprs = []
    for word in words:
        word_length = len(word)
        # replace 함수를 사용하여 키워드 제거 후 길이 차이로 등장 횟수 계산
        count_expr = (length('content_text') - length(expr(f"replace(content_text, '{word}', '')"))) / word_length
